    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
        <div className="max-w-md w-full bg-white rounded-[48px] shadow-2xl p-12 text-center space-y-10 animate-in fade-in zoom-in duration-500 border border-white">
          ...
          {storageError && (
            <div className="bg-rose-50 p-6 rounded-[28px] border border-rose-100 text-left space-y-3">
              <div className="flex items-center space-x-2 text-rose-600 font-bold">
                <AlertCircle size={18} />
                <span className="text-sm">보안 제한 발생</span>
              </div>
              <p className="text-xs text-rose-500 leading-relaxed font-medium">
                {storageError}
              </p>
              {storageError.includes('프레임') && (
                <button 
                  onClick={() => window.open(window.location.href, '_blank')}
                  className="w-full flex items-center justify-center space-x-2 py-3 bg-rose-600 text-white rounded-xl text-xs font-bold hover:bg-rose-700 transition-colors shadow-lg shadow-rose-100"
                >
                  <ExternalLink size={14} />
                  <span>새 탭으로 열기 (권장)</span>
                </button>
              )}
            </div>
          )}
          ...
          <div className="space-y-3">
            <button 
              ...
              className="w-full bg-indigo-600 text-white font-black py-5 rounded-[24px] shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center justify-center space-x-3 active:scale-[0.98]"
            >
              <FolderOpen size={20} />
            ...
            <button 
              ...
              className="w-full bg-white text-gray-500 font-bold py-4 rounded-[24px] border border-gray-100 hover:bg-gray-50 transition-all text-sm"
            >
              임시 브라우저 저장소 사용
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
               ...
               {Array.from({ length: 42 }).map((_, i) => {
                 ...
                 return (
                    <div 
                      ...
                      className={`min-h-[100px] md:min-h-[140px] p-3 border-r border-b border-gray-50 cursor-pointer transition-all ${!isCurrentMonth ? 'opacity-20 bg-gray-50' : 'bg-white'} ${isSelected ? 'bg-indigo-50/50 ring-2 ring-inset ring-indigo-500/10' : 'hover:bg-slate-50'}`}
                    >
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 text-white rounded-[40px] p-10 shadow-2xl relative overflow-hidden">
            ...
            <Moon className="absolute -right-10 -bottom-10 text-white opacity-10 rotate-12" size={180} />
          </div>
          ...
          <div className="bg-white rounded-[40px] shadow-2xl shadow-gray-200/40 p-8 border border-gray-50">
            ...
            <div className="space-y-4">
              <input 
                ...
                placeholder="오늘의 생각은?"
                className="w-full bg-gray-50 rounded-2xl py-4 px-6 text-sm font-bold focus:bg-white focus:ring-4 focus:ring-indigo-100 transition-all outline-none"
              />
              <div className="flex gap-2">
                {[MemoType.TODO, MemoType.IDEA, MemoType.APPOINTMENT].map(type => (
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all ${selectedType === type ? 'bg-indigo-600 text-white shadow-lg' : 'bg-gray-50 text-gray-400'}`}
                  >
                    {type}
                  </button>
                ))}
            ...
            <div className="space-y-4">
              {currentDayMemos.map(memo => (
                <div key={memo.id} className="group flex items-start justify-between bg-slate-50/50 p-5 rounded-3xl border border-transparent hover:border-indigo-100 hover:bg-white transition-all shadow-sm">
                  <div className="flex items-start space-x-4 flex-1">
                    <button onClick={() => handleToggleMemo(memo.id)} className="mt-1">
                      {memo.completed ? <CheckCircle2 className="text-emerald-500" size={20} /> : <Circle className="text-gray-300" size={20} />}
                    </button>
              ...
              {currentDayMemos.length === 0 && (
                <div className="text-center py-20 text-gray-300 space-y-2">
                  <ListTodo size={40} className="mx-auto opacity-20" />
                  <p className="text-xs font-bold">기록이 없습니다.</p>
                </div>
              )}
            </div>
          </div>
