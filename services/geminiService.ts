...
export async function getDailyFortune(birthDate: string, birthTime: string, targetDate: string) {
  ...
  if (!apiKey) {
    console.warn("API_KEY가 설정되지 않았습니다. Vercel 환경 변수 혹은 index.html의 shim을 확인하세요.");
    return "API 키가 설정되지 않아 운세를 불러올 수 없습니다. 프로필 설정에서 알림 권한과 정보를 확인해주세요.";
  }
...
마지막에는 오늘의 행운의 색과 행운의 숫자를 추천해주세요.`;
    ...
    return "AI 운세 서비스를 불러오는 중 오류가 발생했습니다. API 키의 유효성을 확인해주세요.";
  }
}
